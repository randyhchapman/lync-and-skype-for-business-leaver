#####################################################################################################
# Disable-ADSFBCert.ps1
#
# V1.0 - Monday 14th September 2015 - This script asks for the name of a user account of a 
# person that has left the company.  It disables the account in Active Directory, Disables
# the account in Skypoe for Business and revoves the user certificate. (http://lynciverse.com)
# 
# The script also creates a creates and writes the process to a log file
#
# Example usage.
#
# .\Disable-ADSFBCert.ps1 -verbose $true
#
#####################################################################################################
param($verbose)
...
#Create the Log File
$LogFile = "Disable-ADSFBCert-"+(Get-Date -format d.M.yyyy-HHmm)+".txt"
$LogTXT = "Starting operation"
Out-File -FilePath $LogFile -InputObject $LogTXT

# Assigning the variables
$userAccount = Read-Host "Enter the samAccountName of the account you want to disable. E.g. First.Last"
$dn = Get-ADUser $userAccount | Select -ExpandProperty DistinguishedName
$name = Get-ADUser $userAccount | Select -ExpandProperty Name
$ou = Choose-ADOrganizationalUnit

# Writing to the log file
$LogTXT = "Moving AD Object $dn to $ou"
$LogTXT = "Disabling user account $name"
$LogTXT = "All granted certificates have been revoked for user $name"
Out-File -FilePath $LogFile -InputObject $LogTXT -Append

foreach ($user in $userAccount)
{
    Get-ADObject $dn | Move-AdObject -TargetPath $ou
    Disable-ADAccount $userAccount
     
    if ($verbose -eq $true)
    {
        Write-Host "User account $name has now been moved to the $ou OU, disabled in Active Directory & Skype for Business and has had their Certificate revoked."
	Write-Host "Moving AD Object $dn to $ou"
	Write-Host "Disabling user account $name"
    }

    Revoke-CsClientCertificate -Identity $userAccount

    $LogTXT = "User account $name has now been moved to the $ou OU, disabled in Active Directory & Skype for Business and has had their Certificate revoked."
    Out-File -FilePath $LogFile -InputObject $LogTXT -Append
}